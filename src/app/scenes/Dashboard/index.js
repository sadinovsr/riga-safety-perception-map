      ...
      <Table bordered>
        <thead>
          <tr>
            ...
            {regions.map((region) => (
              <th colSpan={6} style={{borderRight: 'solid 2px black'}}>
                {region.displayName}
              </th>
            ))}
          </tr>
          <tr>
            {regions.map((region) => (
              <Fragment>
                ...
                <th style={{borderRight: 'solid 2px black'}}>
                  Redzamība
                </th>
              </Fragment>
            ))}
        ...
        <tbody>
          {completedResponses.map((response) => (
            <tr>
              ...
              <td>
                {new Date(response.time * 1000).toISOString().substr(11, 8)}
              </td>
              <td>
                {this.genderConvert(response.sex)}
              </td>
              <td>
                {response.age}
              </td>
              <td>
                {this.liveInRigaConvert(response.liveInRiga)}
              </td>
              <td>
                {this.rigaRegionConvert(response.whereInRiga) || '-'}
              </td>
