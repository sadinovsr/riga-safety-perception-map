...
class Residence extends Component {
  ...
  render() {
    ...
    const insideOptions = unfilteredData.header.whereInRigaOptions
      .map((region, idx) => ({
        value: idx+1,
        label: `${region} (${this.countByInsideOutsideRegion('inside', idx+1)})`,
      }));
    const outsideOptions = unfilteredData.header.whereOutRigaOptions
      .map((region, idx) => ({
        value: idx+1,
      ...
      <Row>
        ...
        <FormGroup className="d-flex flex-column align-items-start whiteText">
          <div>
            ...
            />
            <Label>
              {`Rīgā (${this.countByInsideOutside('inside')})`}
            </Label>
          </div>
          <div>
            <Input
            ...
            />
            <Label>
              {`Ārpus Rīgas (${this.countByInsideOutside('outside')})`}
            </Label>
          </div>
        </FormGroup>
        <FormGroup>
          ...
          />
          <Label className="whiteText mt-1">
            Ārpus Rīgas reģioni
          </Label>
          <Select
            isMulti
